...
export default async function AdminHome() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl md:rounded-3xl bg-gradient-to-br from-brand-700 via-brand-800 to-brand-900 text-white p-5 md:p-10 shadow-xl">
        ...
        <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <h1 className="mt-3 text-xl md:text-3xl font-extrabold">أهلاً بك في لوحة التحكم</h1>
            <p className="text-white/80 mt-1 text-sm md:text-base max-w-lg">
              من هنا تدير بيانات المنصة وتُصدر التقارير للمستخدمين
            </p>
          </div>
          <Link
            href="/admin/analyze"
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-white text-brand-800 hover:bg-gold-50 px-5 py-3 font-bold shadow-lg shadow-black/10 transition hover:scale-105 active:scale-95 w-full md:w-auto"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
        {cards.map((c) => (
          <Link
            key={c.href}
            href={c.href}
            className="group relative overflow-hidden rounded-2xl bg-white border border-stone-100 p-4 md:p-5 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300"
          >
            <div className={`absolute inset-0 bg-gradient-to-br ${c.color} opacity-0 group-hover:opacity-[0.04] transition-opacity`} />
            <div className={`w-10 h-10 md:w-12 md:h-12 rounded-xl ${c.bg} flex items-center justify-center group-hover:scale-110 transition-transform`}>
              <c.icon className="w-5 h-5 md:w-6 md:h-6 text-stone-700" />
            </div>
            <div className="mt-3 md:mt-4 relative">
              <div className="text-2xl md:text-3xl font-extrabold text-stone-900">{c.count}</div>
              <div className="text-xs md:text-sm text-stone-600 mt-1 flex items-center justify-between">
                {c.label}
